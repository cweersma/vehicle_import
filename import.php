...
switch ($info_type){
    ...
    case "--use-spec":
        ...
        $inserts[] = "INSERT INTO l_models (make_id, model_name) SELECT l_makes.make_id, t_sv.model_name FROM t_sv INNER JOIN l_makes using (make_name) LEFT JOIN l_models ON l_makes.make_id = l_models.make_id AND t_sv.model_name = l_models.model_name WHERE model_id IS NULL";
        $inserts[] = "INSERT INTO l_engine_types (engine_type_name) SELECT engine_type FROM t_sv LEFT JOIN l_engine_types ON t_sv.engine_type = l_engine_types.engine_type_name WHERE engine_type_id IS NULL";
